...
class Agent():
    ...
    def draw(self):
        pygame.draw.rect(screen, (self.red, self.green, self.blue), self.body.move(self.position[0], self.position[1]))
    
    def move(self, direction):
        if not self.body.move(self.position[0], self.position[1]).colliderect(obstacle.body.move(obstacle.position[0], obstacle.position[1])):
            if direction == "up":
                self.position[1] -= 5
            elif direction == "down":
                self.position[1] += 5
...
agent = Agent()
win = Win()
obstacle = Obstacle(400, 250, 20, 100)
